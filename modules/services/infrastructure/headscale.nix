    ...
    systemd.services.headscale-api-key-init = mkIf (cfg.apiKeyFile == null) {
      ...
      script = ''
        ...
        if [ ! -f /var/lib/headscale/.api-key-generated ]; then
          ...
          echo "  cd /data/tristonyoder/Projects/nix-config/secrets"
          echo "  sudo cat /var/lib/headscale/api-key.txt > /tmp/api-key.txt"
          echo "  ./encrypt-secret.sh -n headscale-api-key.age -f /tmp/api-key.txt --git-add"
          echo "  rm /tmp/api-key.txt"
          echo ""
